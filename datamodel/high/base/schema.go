...
type Schema struct {
	...
	DependentSchemas  typex.Pairs[string, *SchemaProxy] `json:"dependentSchemas,omitempty" yaml:"dependentSchemas,omitempty"`
	PatternProperties typex.Pairs[string, *SchemaProxy] `json:"patternProperties,omitempty" yaml:"patternProperties,omitempty"`
	...
	Properties           typex.Pairs[string, *SchemaProxy] `json:"properties,omitempty" yaml:"properties,omitempty"`
	...
	Extensions           typex.Pairs[string, any]          `json:"-" yaml:"-"`
	low                  *base.Schema
...
func NewSchema(schema *base.Schema) *Schema {
	...
	// props async
	buildProps := func(
		low typex.Pair[lowmodel.KeyReference[string], lowmodel.ValueReference[*base.SchemaProxy]],
		dst *typex.Pairs[string, *SchemaProxy],
	) {
		k, v := low.Key, low.Value
		dst.Push(k.Value, &SchemaProxy{
			schema: &lowmodel.NodeReference[*base.SchemaProxy]{
				Value:     v.Value,
				KeyNode:   k.KeyNode,
	...
	for _, p := range schema.Properties.Value {
		buildProps(p, &s.Properties)
	}
	for _, p := range schema.DependentSchemas.Value {
		buildProps(p, &s.DependentSchemas)
	}
	for _, p := range schema.PatternProperties.Value {
		buildProps(p, &s.PatternProperties)
	}
